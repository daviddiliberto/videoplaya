...
function onYouTubePlayerAPIReady() {
	...
	$("#searchButton").html("Add video to playlist");
...
function doThings(){
	...
	$.ajax({
		...
		success: function(response){
			if(response == "ERROR_1"){
				$("#message").html("Add a video!");
				playlistState = "ERROR_1";
			}
			else if(response == "ERROR_2"){
				$("#message").html("Playlist ran out, add more videos...!");
	...
	$.ajax({
		...
		success: function(response){
			if(response == "ERROR_1"){
				...
				$("#message").html("Playlist finished, add more videos...!");
				playlistState = "ERROR_2";
				currentDJ = null;
				setCurrentDJ(" --- ");
			}
